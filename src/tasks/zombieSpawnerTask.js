...
/**
 * Daftar Item Makanan yang Didukung untuk Pemulihan Vitalitas
 */
 ...
 * @description Menyediakan antarmuka seragam antara LiveProtocolClient, Mineflayer, dan MockArenaHarness.
 */
class UnifiedClientAdapter {
  ...
  getPosition() {
    if (this.isLiveClient) {
      return { ...(this.rawClient.position || { x: 0, y: 64, z: 0 }) };
    }
  ...
  setPosition(pos) {
    if (this.isLiveClient) {
      if (typeof this.rawClient.sendPosition === 'function') {
        this.rawClient.sendPosition({ x: pos.x, y: pos.y, z: pos.z, onGround: true });
      } else if (this.rawClient.position) {
        this.rawClient.position.x = pos.x;
        this.rawClient.position.y = pos.y;
        this.rawClient.position.z = pos.z;
      }
  ...
  setHealth(health) {
    if (this.isLiveClient) {
      this.rawClient.health = health;
    } else if (this.botRef) {
      this.botRef.health = health;
    }
  }
  ...
  setFood(food) {
    if (this.isLiveClient) {
      this.rawClient.food = food;
    } else if (this.botRef) {
      this.botRef.food = food;
    }
  }
  ...
  async lookAt(coord) {
    if (this.isLiveClient && typeof this.rawClient.sendPositionAndRotation === 'function') {
  ...
  attack(target) {
    if (this.isLiveClient && typeof this.rawClient.sendAttack === 'function') {
...
class ZombieSpawnerTask extends EventEmitter {
   ...
   * @param {Object} [options.movementController] - Controller pergerakan navigasi opsional
   * @param {{ x: number, y: number, z: number }} [options.targetCoords] - Titik target spawner (default: [-256, -20, -432])
   * @param {{ x: number, y: number, z: number }} [options.retreatCoords] - Titik aman mundur (default: [-256, -20, -420])
   ...
   * @param {number} [options.autoEatThreshold=14] - Ambang batas rasa lapar untuk makan otomatis (Food <= 14)
   * @param {number} [options.retreatThreshold=6] - Ambang batas darah kritis untuk mundur (HP < 6)
   * @param {number} [options.safeRecoveryHealth=14] - Batas darah aman untuk kembali farming (HP >= 14)
   ...
   * @param {number} [options.tickIntervalMs=50] - Interval tick loop evaluasi FSM
   */
  constructor(options = {}) {
    super();
    ...
    // Penjejakan Target Aktif & Mock Queue
    this.activeTarget = null;
    this.mockTargetQueue = [];

    // Cooldown Senjata Terkonfigurasi (default: 625ms untuk sword)
    this.weaponCooldownMs = WEAPON_COOLDOWNS_MS[this.options.weapon] || WEAPON_COOLDOWNS_MS.default || 625;
  ...
  _setupClientEventListeners() {
    this.adapter.on('health', (vitalityData) => {
  ...
  async start() {
    ...
    // Rekam status awal XP dan Level
    ...
    // Pastikan posisi bot berada di perimeter spawner
  ...
  /**
   * Menambahkan antrean target zombie tiruan untuk skenario pengujian terisolasi.
  ...
  /**
   * Mengembalikan snapshot status tugas lengkap untuk Web Dashboard dan Telemetri.
   * @returns {Object}
   */
  getTaskStatus() {
  ...
  async _handleVitalityManagement() {
    ...
    if (health < this.options.retreatThreshold) {
      if (this.state !== TASK_STATES.RETREATING) {
        ...
        // Pindahkan posisi ke retreat waypoint
        this.adapter.setPosition(this.options.retreatCoords);
      }
      return 'RETREAT';
    }

    // Jika sedang dalam status RETREATING dan darah sudah pulih kembali ke batas aman
    if (this.state === TASK_STATES.RETREATING) {
    ...
    if (food <= this.options.autoEatThreshold || (health < 20 && food < 20)) {
      ...
      if (foodItem) {
        ...
        this.adapter.setFood(restoredFood);
        this.adapter.setHealth(restoredHealth);
  ...
  async _executePacedAttack(target) {
    ...
    // Pastikan cooldown terpenuhi sebelum mengirim tebasan (+5ms buffer untuk akurasi timer)
    if (this.lastAttackTimestamp > 0 && elapsedSinceLastAttack < this.weaponCooldownMs) {
      const waitTimeMs = (this.weaponCooldownMs - elapsedSinceLastAttack) + 5;
